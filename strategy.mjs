...
// CJS avoids the SDK's transitive extensionless ESM imports under Node 22.
...
export const deployment = freeze(
  JSON.parse(readFileSync(new URL('./config/deployment.json', import.meta.url))),
);
export const C = freeze(
  Object.fromEntries(Object.entries(deployment.contracts).map(([k, v]) => [k, v.address])),
);
...
export const aquaAbi = new Interface([
...
export const routerAbi = new Interface(sdk.ABI.SWAP_VM_ABI);
export const json = (x) => JSON.stringify(x, (_, v) => (typeof v === 'bigint' ? String(v) : v), 2) + '\n';
...
export function normalize(config, now = BigInt(Math.floor(Date.now() / 1000))) {
  const keys = [
    'chainId',
    'maker',
    'fwUSDC',
...
export function buildStrategy(config, { now = BigInt(Math.floor(Date.now() / 1000)) } = {}) {
...
export function buildQuote(config, request, { now = BigInt(Math.floor(Date.now() / 1000)) } = {}) {
  ...
  const [tokenIn, tokenOut] = request.direction === 'USDC_USDT' ? [C.fwUsdc, C.fwUsdt] : [C.fwUsdt, C.fwUsdc];
