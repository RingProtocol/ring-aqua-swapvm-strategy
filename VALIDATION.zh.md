...
## 当前 0.2.2：原生转换与代币权限，2026-09-16

**74 项离线测试、103 项主网 fork 测试全部通过。** [本轮 fork 报告](evidence/native-conversions-2026-09-16/fork-results.json)记录对应源文件、lockfile 哈希和固定历史区块。库存为本地模拟，所有写入都发生在专用 Anvil，没有发送主网交易、创建真实资金仓位，也没有证明 1inch 前端接单。

新增 8 项 fork 测试覆盖：ETH/WETH 往返并单独核对 gas；ETH/WETH/fwWETH 逐步包装与反向解包；ETH/WETH 余额不足；WETH、USDT、普通 ERC-20 UNI 在已有非零授权时的包装和解包；Aqua 登记 SDK 目录外的币；有 ERC-20 授权但无对应仓位额度时拒绝转出。USDT 额外确认非零授权直接改非零会失败、approve 返回空数据。原有成交、过期、钱包身份、失败回滚与共享授权用例全部重跑。

本次新增的是做市钱包的未签名 ETH/WETH 转换计划，没有新增生产执行器的原生 ETH 用户订单功能，也没有链上全局代币白名单或管理员。类型、浏览器构建与独立安装包验证覆盖 Node/portable 两个入口；不将构建通过写成新增接口已在真实浏览器运行。[本轮验证记录](evidence/native-conversions-2026-09-16/verification.json)。

此前审核提交 `2cff867` 已推送至 `feat/aqua-integration-sdk`，本次修正沿用该分支；`main` 和仓库公开状态不变。当前 CI 仅在 main 推送或 PR 时触发，不能将审核分支 push 写成远端 CI 通过。下方发布状态均保留为对应日期的历史记录。

## 较早的文档范围整理：2026-09-16
